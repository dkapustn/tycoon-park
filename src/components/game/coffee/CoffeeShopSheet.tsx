...
import { COFFEE_UPGRADES } from '../../../games/coffee/drinks'
import { formatNumber } from '../../../lib/format'
import { sfx } from '../../../lib/sound'
import { haptic } from '../../../lib/haptics'
import { ShopItem } from '../ShopItem'

export function CoffeeShopSheet({ open, onClose }: { open: boolean; onClose: () => void }) {
  const coins = useCoffeeStore((s) => s.coins)
  const upgrades = useCoffeeStore((s) => s.upgrades)
  const buyUpgrade = useCoffeeStore((s) => s.buyUpgrade)
  ...
  const coinUpgrades = COFFEE_UPGRADES.filter((u) => u.currency !== 'diamonds')
  const gemUpgrades = COFFEE_UPGRADES.filter((u) => u.currency === 'diamonds')
        ...
        >
          ...
          >
            ...
            <div className="mb-2 flex items-center justify-between">
              <h2 className="font-display text-lg font-bold">🏗️ Прокачка кофейни</h2>
