...
import { PIZZA_UPGRADES } from '../../../games/pizza/pizzas'
import { formatNumber } from '../../../lib/format'
import { sfx } from '../../../lib/sound'
import { haptic } from '../../../lib/haptics'
import { ShopItem } from '../ShopItem'

export function PizzaShopSheet({ open, onClose }: { open: boolean; onClose: () => void }) {
  const coins = usePizzaStore((s) => s.coins)
  const upgrades = usePizzaStore((s) => s.upgrades)
  const buyUpgrade = usePizzaStore((s) => s.buyUpgrade)
  ...
  const coinUpgrades = PIZZA_UPGRADES.filter((u) => u.currency !== 'diamonds')
  const gemUpgrades = PIZZA_UPGRADES.filter((u) => u.currency === 'diamonds')
        ...
        >
          ...
          >
            ...
            <div className="mb-2 flex items-center justify-between">
              <h2 className="font-display text-lg font-bold">🏗️ Прокачка пиццерии</h2>
