...
import { MINE_UPGRADES } from '../../../games/mine/ores'
import { formatNumber } from '../../../lib/format'
import { sfx } from '../../../lib/sound'
import { haptic } from '../../../lib/haptics'
import { ShopItem } from '../ShopItem'

export function MineShopSheet({ open, onClose }: { open: boolean; onClose: () => void }) {
  const coins = useMineStore((s) => s.coins)
  const upgrades = useMineStore((s) => s.upgrades)
  const buyUpgrade = useMineStore((s) => s.buyUpgrade)
  ...
  const coinUpgrades = MINE_UPGRADES.filter((u) => u.currency !== 'diamonds')
  const gemUpgrades = MINE_UPGRADES.filter((u) => u.currency === 'diamonds')
        ...
        >
          ...
          >
            ...
            <div className="mb-2 flex items-center justify-between">
              <h2 className="font-display text-lg font-bold">🏗️ Снаряжение шахты</h2>
